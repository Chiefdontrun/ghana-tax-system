...
const NAV_ITEMS: NavItem[] = [
  ...
  { to: "/admin/tax/assessments", label: "Tax assessments", icon: <TaxIcon />, taxAdminMin: true },
  { to: "/admin/tax/exceptions", label: "Tax exceptions", icon: <TaxIcon />, taxAdminMin: true },
  { to: "/admin/tax/rate-schedules", label: "Rate schedules", icon: <TaxIcon />, sysAdminOnly: true },
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto" aria-label="Sidebar">
        <ul className="space-y-0.5">
          {NAV_ITEMS.filter((item) => {
            if (item.sysAdminOnly) return role === "SYS_ADMIN";
            if (item.taxAdminMin) return role === "TAX_ADMIN" || role === "SYS_ADMIN";
            return true;
          }).map((item) => (
            <li key={item.to}>
              <NavLink
